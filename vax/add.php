...
<div class="row">
	<div class="col-lg-8 col-lg-push-3">		
			...
			if ($action == 'create') {
			    ...
			    try {     
			        ...
			        $query = "INSERT INTO Vaccines 
		        				SET amiID = ?, vaxDate = ?, vaxDesc = ?, vaxDue = ?";
			        ...
			        $stmt->bindParam(2, $_POST['vaxDate']);
			        $stmt->bindParam(3, $_POST['vaxDesc']);
			        $stmt->bindParam(4, $_POST['vaxDue']);
		...
		<form action='#' method='post' id='addVax'>
			<div class="row">
				<div class="col-md-6">
					<input type="hidden" name="amiID" value="<?php echo $amiID ?>" />
					<label for="vaxDate">Vaccine Date <small>yyyy-mm-dd</small></label>
					<input type='text' name='vaxDate' id="vaxDate" class="datepicker form-control" value=""  />
				</div>

				<div class="col-md-6">
					<label for="vaxDue">Vaccine Due <small>yyyy-mm-dd</small></label>
					<input type='text' name='vaxDue' id="vaxDue" class='datepicker form-control' value="" />
				</div>
			</div>
			<div class="row">
				<div class="col-sm-12 columns">
					<label>Vaccine Description</label>
					<input type="text" name="vaxDesc" class="form-control" value="" />
				</div>
			</div>
			<div class="row">
				<div class="col-sm-12 ">
					...
					<button type="submit" class="btn btn-success tip" title="Click or press Enter to save">
						<i class="fa fa-floppy-o"></i>Save New Vaccine
					</button>
					<a href='index.php?amiID=<?php echo $amiID ?>' class="border-left pad-left margin-left">
						<i class="fa fa-medkit margin-right-small"></i>Vaccine Main Page
					</a>
				</div>
			</div>
		</form>
...
$(document).ready(function () {
	...
	$('#vaxDate').change(function(){ 
		...
		var $nextDue = $nextYear + "-" + $thisMonth + "-" + $thisDay;
		$('#vaxDue').val($nextDue);
	});
